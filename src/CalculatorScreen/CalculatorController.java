...
public class CalculatorController {
    ...
    public void setCurrentNumber(){
        if (Number==""&&mOperetor==""){CurrentNumber.setText("you entered nothing");}
        else{CurrentNumber.setText(mOperetor+Number);}
    }

    @FXML
    public void setHistory(){
        if (mWorking.getOperator()==""){HistoryText+=mWorking.getNum1();}//if its 1st num
        else if (mWorking.getOperator().equals("ChangingSign")){ setOperetorSign();
            HistoryText+=mOperetor+"="+mWorking.getTotalnum();}
        else {setOperetorSign();
            HistoryText+=mOperetor+mWorking.getNum2()+"="+mWorking.getTotalnum();}
        if (HistoryText.length()>40) {char[] temporary=HistoryText.toCharArray();
            HistoryText="... ";
            for(int i=(temporary.length-35);i<temporary.length;i++){HistoryText+=temporary[i];};}
    ...
    private void Operetion(String operetion){
        if(mWorking.getOperator()!=""&&Number.equals("")){
            ...
            setOperetorSign();
            setCurrentNumber();}
        else{mWorking.setNum2(mWorking.SetNum(Number));
            mWorking.setTotalnum(mWorking.totalnum(mWorking.getNum1(),mWorking.getNum2(),mWorking.getOperator()));
            setHistory();
            mWorking.setOperator(operetion);
            setOperetorSign();
    ...
    public void Delete(ActionEvent event) {
        ...
        Number="";
        for(int i=0;i<(temp.length-1);i++){Number+=temp[i];};
        setCurrentNumber();
    }

    public void Equals(ActionEvent event) {Operetion("Equals");}
